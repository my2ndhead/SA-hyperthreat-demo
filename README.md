...
- A user "hrlegal" is created with the role hrlegal. This user has also stored the password for the private key "private_aes256.pem" in the Splunk keystore. NOTE: Due to limitations within the Splunk Password Key Store, hrlegal has/needs the "admin_all_objects"-capability. In real-life this right would be given just temporary. This is an open issue we will work around in a future release.
